...
│   ├── AgentRunsPage.tsx          # 运行、合同、policy、作业和事件
│   ├── ReviewCenterPage.tsx       # 证据绑定的人工评价
...
│   ├── /tasks/:taskId/inspect         真实 SSH 只读探测
│   ├── /upload /download              Task 根内传输
│   └── /submit-smoke /jobs/*          固定 LSF smoke 与对账
│
...
### 现状
- 原有 CRUD 路由仍直接调用 SQL；Agent V1 的合同、策略、路径和远程状态机已拆到 `server/services/`
- 现役数据库有 14 张业务表；`PRAGMA user_version` 管理 Agent V1 增量迁移并生成迁移前备份
- 旧 HPC 页面继续只生成复制命令；真实远程动作只从 Agent V1 受控 adapter 进入，默认环境开关关闭
