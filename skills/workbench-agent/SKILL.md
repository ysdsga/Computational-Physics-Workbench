...
For the `theoretical-research` workflow, new Envelopes default `explorationReviewRequired` to `true`. Before completion, keep `workingPlan.explorationReview` explicit: use `continue` with the unresolved high-value questions or generative ideas that require another derivation/validation cycle; use `passed` only when none remain. A high-value idea remains open while marked `explore`, `deferred`, or `follow_up`; deferring it or naming a later task does not unblock completion. Close it only as `resolved` or `falsified`, with at least one concrete `evidenceRefs` entry identifying the derivation, registered evidence, or Task-relative result that supports closure. Do not treat every possible idea as blocking—only one that could materially change, extend, or overturn the central conclusion.

Before closing each theoretical core stage, actively search for ideas that could materially change, extend, or overturn the central conclusion by checking counterexamples, competing mechanisms, controlled limits, and testable predictions. There is no idea quota: do not invent ideas to satisfy the process. If no high-value idea is found, briefly record which directions were examined; if one is found, enter it into the unresolved-to-evidence-closure lifecycle above.

For theoretical research, read [references/theoretical-research.md](references/theoretical-research.md) before drafting a new Run, exploring routes, or deciding whether to finish. New reviewed Runs require `confirmedEnvelope.scientificGoal`: the original question, success criteria, insufficient outcomes and accepted answer types. The goal persists across Runs; routes and models may change inside the confirmed boundary. Never silently narrow the goal to the conditional or diagnostic result currently available. A changed goal requires an explicit reason, the previous goal hash, and researcher confirmation of the new Envelope.

Maintain a generative outer loop: research map and route search → model and controlled limits → derivation and validation → learn from the result → search again. No current idea is not exhaustion. Search longitudinally, horizontally, through analogies, or through failed assumptions, then attempt a discriminating physical argument. Do not replace this work with more checks or bookkeeping. A route closed as `resolved` or `falsified` remains in the task-wide map with its learning and provenance; closure does not imply the scientific question is answered. Use `workbench research-map show --task <id>` on recovery before repeating or extending earlier work.

During and at the end of every theoretical core stage, append a reflection with `workbench reflection record` whenever a finding materially changes the next decision; there is no per-stage count limit. State what was established, remaining uncertainties, every material new idea and its disposition (`explore`, `resolved`, `falsified`, `deferred`, or `follow_up`), and the next decision. When changing an earlier idea's disposition, repeat the same `idea` text so its lifecycle remains machine-trackable, and include `evidenceRefs` for `resolved` or `falsified`. `proceed` closes the current stage and advances to the next confirmed stage, `stay` keeps the current stage for another reflection, and `loop` returns to the earliest confirmed stage affected by the finding. The reflection atomically updates the Working Plan target and next Actions; do not separately overwrite or omit the durable reflection history. This stage-reflection mechanism does not by itself modify the Research Plan.
...
For a theoretical Run with exploration review enabled, do not call `run complete` until every core stage has a current `proceed` reflection and the exploration review is `passed`. A later `stay` or `loop` invalidates the affected stage and all downstream stage closeouts until they are revisited. The interpretation reflection passes the global exploration review only when no high-value idea remains unresolved, including ideas marked `explore`, `deferred`, or `follow_up`.

For Runs with `scientificGoal`, interpretation and final release must additionally assess every original success criterion through `goalAssessment`. Only an evidenced answer of an accepted type, without remaining central gaps, permits success. A conditional theorem can be a valid goal when explicitly agreed; it cannot stand in for an explanation that still assumes the effect to be explained. Partial results must loop; lack of novelty or lack of candidates returns to route exploration, while a failed assumption or missing causal link returns to model/derivation. A real resource/authority limit may use a blocking researcher pending item (`blocksRun: true`) to preserve partial progress without successful completion. Do not invent a boundary merely because the next idea is unknown. No new stages, action quotas, automatic software installation, or perpetual automation are implied.
