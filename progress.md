...
> 核对日期：2026-08-16
> 当前实现：Essential Workbench V3 / SQLite schema v5。
...
- `npm test` 通过：12 passed，1 个 Windows symlink 用例按环境跳过。
- 临时库覆盖 schema v5、新 Task 不创建 Stage 目录、一次 Envelope 确认、Working Plan、自主本地 Action、阶段级 Pending、Artifact 纠错、Experience candidate、LSF single-shot 与 uncertain reconcile。
