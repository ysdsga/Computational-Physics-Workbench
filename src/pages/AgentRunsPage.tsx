...
export default function AgentRunsPage() {
  ...
  return <div className="h-full overflow-y-auto bg-[#0d1214] text-[#e9f0ed]">
    ...
    <main className="space-y-5 p-8">
      ...
      <section className="grid gap-px border border-[#293534] bg-[#293534] sm:grid-cols-5">{[
        ['Run', context?.run?.status ?? '未开始'], ['Stage', context?.run?.current_stage_id ?? '—'], ['Actions', String(counts.actions)], ['Jobs', String(counts.jobs)], ['待沟通', String(counts.pending)],
      ...
      <section className="border border-[#293534] bg-[#131a1c] p-5"><div className="flex items-center justify-between"><h2 className="flex items-center gap-2 font-medium"><Server size={17} className="text-[#dfb26a]"/>远程作业</h2><span className="flex items-center gap-1 font-mono text-[10px] text-[#657570]"><Clock3 size={12}/>显示最近一次调度器观察，不是实时终端</span></div><div className="mt-4 overflow-x-auto"><table className="w-full min-w-[720px] text-left text-xs"><thead className="font-mono text-[9px] uppercase text-[#657570]"><tr>{['Job', 'Stage / Action', 'Host / Workdir', 'State', 'Scheduler ID', 'Reconciled'].map(label => <th key={label} className="border-b border-[#354341] px-3 py-2">{label}</th>)}</tr></thead><tbody>{context?.recentJobs.map(job => <tr key={job.id} className="border-b border-[#24302f] text-[#bac7c3]"><td className="px-3 py-3 font-mono">{job.job_name}</td><td className="px-3 py-3">{job.stage_id}<br/><span className="font-mono text-[9px] text-[#657570]">{short(job.action_id)}</span></td><td className="px-3 py-3 font-mono text-[10px]">{job.host}<br/>{job.remote_workdir}</td><td className="px-3 py-3 text-[#dfb26a]">{job.status}</td><td className="px-3 py-3 font-mono">{job.job_id ?? 'unconfirmed'}</td><td className="px-3 py-3 font-mono text-[10px]">{job.reconciled_at ?? '从未对账'}</td></tr>)}</tbody></table>{!context?.recentJobs.length && <div className="p-8 text-center text-sm text-[#657570]">没有已记录的远程作业。</div>}</div></section>
